    ...
    mainPanel( #poistion="right",
      tabsetPanel(
        ...
        tabPanel(
          ...
          h4("Annual Seasonality Test Results"),
          verbatimTextOutput("annualseasonal"),
          h4("ADF Test Results"),
          verbatimTextOutput("adftest"),
          h4("Cointegration Test"),
          verbatimTextOutput("coint")
        ),
        tabPanel(
          "ARIMA & GARCH Analysis",
          h3("Arima Models"),
          verbatimTextOutput("arima"),
          h4("Arima Forecast for next 10 Periods"),
          verbatimTextOutput("arima_forecast"),
          h3("GARCH Model (1,1)"),
        ...
        tabPanel(
          ...
          h3("VAR Model Predictions"),
          verbatimTextOutput("var_pred"),
          h3("Cointegration Test"),
          verbatimTextOutput("coint"),
          h3("Random Stuff"),
...
server <- function(input, output) {
  ...
  garch_model <- function(input, output)
    ({
      ...
      if (input == TRUE)
        ({
          for (i in 1:n()) {
            x[,i] <- fGarch::garchFit(~garch(1,1),data=returns_log()[,i], trace = FALSE)
  ...
  output$coint <- renderPrint(ca.jo(returns_log(), type = c("eigen", "trace"), ecdet = c("none", "const", "trend"), K = 2, spec=c("longrun", "transitory"), season = NULL, dumvar = NULL))
  ...
  output$as <- renderPrint(MTSdiag(var_model(input$var)))
